...
class View(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # image_logo
        self.img_logo = ImageTk.PhotoImage(Image.open("foto.PNG"))
        ...
        self.lbl_light_value = Label(self.frame_light_text, text='700 Lux', bg='white', fg='#474747')
        ...
        self.thread_env = threading.Thread(target=thread_env,args=(self.lbl_temp_value, self.lbl_hum_value,self.lbl_co_value))
        ...
        if "Postura" in value[0]:
            if value[0]['Postura'] == "correcta":
                ...
                if value[0]['Postura'] != lasted_value:
                    ...
                    sensor_1_img = ImageTk.PhotoImage(image_sensor_1_resize)
                    image.config(image=sensor_1_img)
                    status.config(text="Status desconhecido", fg="#474747")
                    """
                    variavel = 1
        ...
        if "Postura" in value[0]:
            if value[0]['Postura'] == "correcta":
                ...
                if value[0]['Postura'] != lasted_value:
                    ...
                    sensor_2_img = ImageTk.PhotoImage(image_sensor_2_resize)
                    image.config(image=sensor_2_img)
                    status.config(text="Por favor, ajuste a sua posição", fg="#d16d6a")
                    """
...
def thread_battery_1(label):
    ...
    while True:
        ...
        if "battery_level" in value:
            label.config(text=("Nivel da bateria: " + value['battery_level'] + "%"))
        if end is True:
            break
...
def thread_env(label_temperature, label_humidity, label_co2):
    global i
    global end
    global env
    while True:
